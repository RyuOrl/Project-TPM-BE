...
<body>
    ...
    <form action="/update/post/{{$tasks->id}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('post')
        <div class="mb-3">

            <label style="margin-left: 1%; font-weight:bold; "exampleInputEmail1" class="form-label">Name</label>
            <input type="text" name="task_input" value="{{$tasks->task}}"class="form-control" id="exampleInputEmail1"
        ...
        <div class="mb-3">
            ...
            <input type="date" name="due_input" value="{{$tasks->due}}" class="form-control" id="exampleInputPassword1">
            @error('due_input')
                    <div class="alert alert-danger">{{$message}}</div>
                @enderror
        </div>
        <div class="mb-3">
            ...
            <select name="subject_input" id="exampleInputPassword1">
                ...
                <option value="{{ $item->id }}" @if ($item->id == $tasks->subject_id) selected @endif>
                    {{ $item->subject }}</option>
            @empty
        ...
        <div class="mb-3">
            ...
            <input type="text"name="description_input" value="{{$tasks->description}}"class="form-control" id="exampleInputPassword1">
        ...
        <div class="mb-3">
            ...
            <input type="file" name="task_image_input" value="{{$tasks->task_image_path}}" class="form-control" id="exampleInputPassword1">
            @error('task_image_input')
                    <div class="alert alert-danger">{{$message}}</div>
                @enderror
        </div>
        <button style="margin-left: 1%"type="submit" class="btn btn-primary">Update</button>
    </form>
